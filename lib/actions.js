...
import { get, insertedId, run, saveSessionAwards } from "@/lib/db";
...
export async function savePointCategory(formData) {
  await run(`
    ...
    WHERE id = ? AND category_key != 'participation'
  `, [
    text(formData, "label"),
...
export async function saveSessionPoints(formData) {
  ...
  for (const [field, value] of formData.entries()) {
    const match = /^cat_(\d+)_(.+)$/.exec(field);
    if (match && value) selected.add(`${Number(match[1])}:${match[2]}`);
  }
  if (!await saveSessionAwards(sessionId, selected)) return;
